// Make sure event is added only once
...
let STAR_TRAIL_ENABLED = false;

chrome.storage.local.get('star-trail-enabled', result => {
  STAR_TRAIL_ENABLED = result['star-trail-enabled'];
});

chrome.runtime.onMessage.addListener(message => {
  if (message.action !== 'toggle-star-trail') return;
  STAR_TRAIL_ENABLED = message.value;
});
